...
async function bootstrap() {
  ...
  await dataSource.query(
    'TRUNCATE TABLE journal_entries, transactions, account_period_balances, budget_items, budgets, periods, accounts, currencies, users RESTART IDENTITY CASCADE',
  );
  ...
  switch (scenario) {
    case 'base':
      ...
      break;
    case 'default':
      await defaultScenario(em, balanceUpdateService);
      break;
    default:
      console.error(
        `[SEEDER] Escenario desconocido: "${scenario}". Opciones válidas: "default", "base".`,
      );
      process.exit(1);
  }
