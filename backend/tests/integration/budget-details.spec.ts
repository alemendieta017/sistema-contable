...
describe('Budget Details and Items Update Integration Tests', () => {
  ...
  describe('GetBudgetDetailUseCase', () => {
    it('should return budget details and eligible accounts with saved amounts', async () => {
      ...
      const accounts = [
        { id: 'acc-1', name: 'Inversión', type: 'ASSET', isCashOrBank: false, parentId: 'parent-1', status: 'ACTIVE' },
        { id: 'acc-2', name: 'Comida', type: 'EXPENSE', isCashOrBank: false, parentId: null, status: 'ACTIVE' },
      ];
    ...
    it('should dynamically create a budget if it does not exist but the period exists and belongs to the user', async () => {
      ...
      expect(result).toBeDefined();
      expect(mockEntityManager.create).toHaveBeenCalledWith(BudgetEntity, expect.objectContaining({
        userId,
        periodId,
        name: 'Junio 2026',
      }));
  ...
  describe('UpdateBudgetItemsUseCase', () => {
    it('should batch insert/update budget items for an open period', async () => {
      ...
      mockEntityManager.find.mockResolvedValueOnce([
        { id: 'acc-1', name: 'Alquiler', type: 'EXPENSE', isCashOrBank: false, status: 'ACTIVE', userId },
        { id: 'acc-2', name: 'Ahorro', type: 'ASSET', isCashOrBank: false, status: 'ACTIVE', userId },
      ]);
    ...
    it('should throw BadRequestException if trying to update budget items in a CLOSED period', async () => {
      ...
      await expect(updateItemsUseCase.execute(userId, periodId, dto)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw BadRequestException if any account is ineligible (e.g. cash/bank or EQUITY)', async () => {
      ...
      const dto = {
        items: [
          { accountId: 'acc-equity', amount: 1000 },
        ],
      };
      ...
      mockEntityManager.find.mockResolvedValueOnce([
        { id: 'acc-equity', name: 'Patrimonio', type: 'EQUITY', isCashOrBank: false, status: 'ACTIVE', userId },
      ]);
