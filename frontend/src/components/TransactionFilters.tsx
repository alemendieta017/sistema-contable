...
export function isFullMonth(start: string, end: string) {
...
export function MonthNavigator({
  ...
  onDateRangeChange,
  view,
}: Pick<TransactionFiltersProps, 'startDate' | 'endDate' | 'onDateRangeChange' | 'view'>) {
  ...
  const handlePrev = () => {
    ...
    if (view === 'monthly') {
      const currentYear = isNaN(currentDate.getTime())
  ...
  return (
    <div className="sm:hidden bg-white dark:bg-slate-800 -mx-4 border-b border-slate-100 dark:border-slate-800 shadow-sm flex flex-col -mt-4">
      ...
      <div className="p-2 flex items-center justify-between gap-2 text-xs w-full">
        ...
        <div className="flex items-center text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider select-none pl-1">
        ...
        <button
          ...
          className={`flex items-center gap-1 px-2.5 py-1.5 font-bold rounded-xl border transition cursor-pointer text-xs ${
            showAdvanced
              ...
              : props.selectedAccountId || isCustomRangeActive
      ...
      {/* Expanded panel */}
      {showAdvanced && <FilterDrawer {...props} />}
    </div>
  );
}
