...
export default function CashFlowForecastPage() {
  ...
  const { queryStartPeriod, queryMonths, isRolling } = useMemo(() => {
    ...
    if (viewMode === 'annual' || viewMode === 'monthly') {
      // Cargar los 12 meses del ejercicio para disponer de toda la información en memoria con acumulación exacta
  ...
  // Months filtered for grid rendering (in monthly mode, show only the active month)
  ...
  // Compute Hero Summary KPI Totals dynamically from visible months data (Desktop)
  const heroSummary = useMemo(() => {
    ...
    const initialCash = targetMonths[0].initialCash || 0;
    const totalEntradas = targetMonths.reduce((acc, m) => acc + (m.totalEntradas || 0), 0);
    const totalSalidas = targetMonths.reduce((acc, m) => acc + Math.abs(m.totalSalidas || 0), 0);
    const netFlow = totalEntradas - totalSalidas;
    const finalCash = targetMonths[targetMonths.length - 1].finalCash || 0;
  ...
  return (
    <div className="flex flex-col h-full w-full p-2 sm:p-4 space-y-3 font-sans overflow-hidden">
      ...
      {/* 2. Hero Summary Bar (5 KPIs - Desktop Only) */}
      {!isMobile && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 shrink-0">
          ...
          {/* Salidas Totales (sin signo negativo) */}
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-3 shadow-2xs flex flex-col justify-between">
            <div className="flex items-center justify-between text-slate-400 text-xs font-medium">
            ...
            <p className="text-sm sm:text-base font-bold text-rose-600 dark:text-rose-400 mt-1 truncate tabular-nums">
              {formatCurrency(Math.abs(heroSummary.totalSalidas), baseCurrency)}
            </p>
          </div>
