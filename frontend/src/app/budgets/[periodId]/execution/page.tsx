    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-16">
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700 shadow-sm space-y-6">
          <div className="border-b border-slate-100 dark:border-slate-700 pb-3">
            ...
            <p className="text-3xs text-slate-400 mt-0.5">
              Comparación de Ingresos y Gastos acumulados
            </p>
          </div>

          {/* Income Grid */}
          <div className="space-y-3">
            <h4 className="text-3xs font-extrabold uppercase tracking-wider text-slate-400">
              Ingresos
            </h4>
              ...
              <table className="w-full text-xs text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-100 dark:border-slate-700 text-slate-400 text-3xs font-bold uppercase tracking-wider">
                    ...
                    <th className="py-2 text-right">Ejecutado Real</th>
                    <th className="py-2 text-right">Variación Absoluta</th>
                    <th className="py-2 text-right">Variación Porcentual</th>
                    <th className="py-2 text-center w-16">Alerta</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
                  {consumos.income.map((item) => {
                    ...
                    return (
                      <tr key={item.accountId}>
                        ...
                        <td
                          className={`py-2.5 text-right font-bold whitespace-nowrap ${item.isNegativeDeviation ? 'text-red-500' : 'text-green-500'}`}
                        >
                          {item.deviation > 0 ? '+' : ''}
                        ...
                        <td className="py-2.5 text-center">
                          {item.isNegativeDeviation ? (
                            <span className="inline-flex items-center justify-center p-1 bg-red-50 dark:bg-red-950/20 text-red-500 rounded-lg">
                  ...
                  <tr className="font-extrabold border-t-2 border-slate-100 dark:border-slate-700">
                    ...
                    >
                      {consumos.totalRealIncome - consumos.totalBudgetedIncome > 0 ? '+' : ''}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
                  {consumos.expense.map((item) => {
                    ...
                    return (
                      <tr key={item.accountId}>
                        ...
                        <td className="py-2.5 text-right font-semibold text-slate-600 dark:text-slate-400 whitespace-nowrap">
                          {formatNum(item.real)}
                        </td>
                        <td
                          className={`py-2.5 text-right font-bold whitespace-nowrap ${item.isNegativeDeviation ? 'text-red-500' : 'text-green-500'}`}
                  ...
                  <tr className="font-extrabold border-t-2 border-slate-100 dark:border-slate-700">
                    <td className="py-2.5 whitespace-nowrap">Total Gastos</td>
                    ...
                    >
                      {consumos.totalRealExpense - consumos.totalBudgetedExpense > 0 ? '+' : ''}
                      {formatNum(consumos.totalRealExpense - consumos.totalBudgetedExpense)}
                    </td>
                    <td
                      className={`py-2.5 text-right whitespace-nowrap ${consumos.totalRealExpense > consumos.totalBudgetedExpense ? 'text-red-500' : 'text-green-500'}`}
                    >
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700 shadow-sm space-y-4">
          <div className="border-b border-slate-100 dark:border-slate-700 pb-3">
            ...
            <p className="text-3xs text-slate-400 mt-0.5">
              Control de movimientos de capital invertido o ahorrado
            </p>
          </div>
                ...
                {savings.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="py-4 text-center text-slate-400">
                      No hay planificación de activos para este mes.
                    </td>
                  </tr>
                ) : (
                  savings.map((item) => {
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700 shadow-sm space-y-4">
          <div className="border-b border-slate-100 dark:border-slate-700 pb-3">
            ...
            <p className="text-3xs text-slate-400 mt-0.5">
              Control de pagos de cuotas y toma de préstamos
            </p>
          </div>
            ...
            <table className="w-full text-xs text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
                {debts.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="py-4 text-center text-slate-400">
                      No hay planificación de pasivos para este mes.
                    </td>
                  </tr>
                ) : (
                  debts.map((item) => {
                    ...
                    return (
                      <tr key={item.accountId}>
                        ...
                        <td
                          className={`py-2.5 text-right font-bold ${item.isNegativeDeviation ? 'text-red-500' : 'text-green-500'}`}
                        >
                          {item.deviation > 0 ? '+' : ''}
                          {formatNum(item.deviation)}
                        </td>
      ...
      <div className="bg-gradient-to-tr from-slate-800 to-slate-900 dark:from-slate-900 dark:to-slate-950 text-white rounded-3xl p-6 shadow-xl border border-slate-700/40 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 border-b border-slate-700/50 pb-5 text-xs">
          <div>
            <p className="text-slate-400 font-semibold text-3xs uppercase tracking-wider">
              Saldo Inicial de Caja
            </p>
            <p className="font-extrabold text-base mt-1 text-slate-200 whitespace-nowrap">
              {formatNum(resumenLiquidez.saldoCajaInicialReal)}
            </p>
          </div>
          <div>
            <p className="text-slate-400 font-semibold text-3xs uppercase tracking-wider">
              Flujo Neto Consumos (P&L)
            </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-1 text-xs">
          ...
          <div>
            <p className="text-indigo-300 font-bold text-3xs uppercase tracking-wider">
              Saldo Final de Caja Proyectado vs Real
            </p>
            <div className="grid grid-cols-2 gap-4 mt-2">
              <div className="bg-slate-800/40 p-2.5 rounded-xl border border-slate-700/30">
              ...
              <div className="bg-indigo-950/40 p-2.5 rounded-xl border border-indigo-900/40">
                ...
                <span className="font-extrabold text-sm text-green-400 whitespace-nowrap">
