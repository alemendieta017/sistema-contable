...
export default function BudgetsPage() {
  ...
  useEffect(() => {
    api.periods.list().then((list) => {
      setPeriodsList(list || []);
    }).catch(() => {});
  }, []);

  const getPeriodId = () => {
    const found = periodsList.find((p) => p.name === period);
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 px-4 py-8 max-w-md mx-auto">
      ...
      <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 mb-4 flex justify-between items-center shadow-sm">
      ...
      <div className="flex gap-2.5 mb-6">
        <button
          onClick={() => {
            const id = getPeriodId();
            if (id) {
              window.location.href = `/budgets/${id}/edit`;
            } else {
              alert("Período no encontrado en el Ejercicio Fiscal activo.");
            }
          }}
          className="flex-1 py-2.5 px-4 bg-indigo-600 hover:bg-indigo-750 text-white font-bold rounded-xl text-xs text-center shadow-md transition"
        >
        ...
        <button
          onClick={() => {
            const id = getPeriodId();
            if (id) {
              window.location.href = `/budgets/${id}/execution`;
            } else {
              alert("Período no encontrado en el Ejercicio Fiscal activo.");
            }
          }}
          className="flex-1 py-2.5 px-4 bg-slate-100 hover:bg-slate-250 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-750 dark:text-slate-200 font-bold rounded-xl text-xs text-center border border-slate-200 dark:border-slate-700 transition"
        >
          Ver Ejecución Real
        </button>
      </div>
