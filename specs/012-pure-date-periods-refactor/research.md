...
- _Why rejected_: Highly complex. Every SQL query performing date grouping or filters must receive the user's timezone offset and convert the timestamp using `AT TIME ZONE`. This makes caching difficult, slows down database aggregations, and increases the likelihood of bugs on DST transition days.

### Alternative B: Store UTC midnight timestamps

- _Why rejected_: Even if stored as UTC midnight (e.g. `2026-06-30T00:00:00.000Z`), local client code or libraries converting it to local `Date` objects will drift it (e.g., displaying `2026-06-29T20:00:00` in Paraguay). Keeping the type as `DATE` (string in TS) prevents JS `Date` parser from mutating the values.
